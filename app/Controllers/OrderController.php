...
class OrderController
{
    ...
    public function show($hash, Request $request, Response $response, Twig $view, Customer $customer, Order $order)
    {
        $order = $order->with(['address', 'products', 'customers'])->where('hash', $hash)->first();
    ...
    public function create(Request $request, Response $response, Customer $customer, Address $address)
    {
        ...
        $order = $customer->orders()->create([
            'hash' => $hash,
            'paid' => false,
            'total' => $this->basket->subTotal() + 16.35,
        ...
        $result = Braintree_Transaction::sale([
            'amount' => $this->basket->subTotal() + 16.35,
